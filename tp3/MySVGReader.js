...
export class MySceneGraph {
	...
	parseXMLFile(rootElement) {
		...
		if ((index = nodeNames.indexOf("g")) == -1)
			...
			//Parse scene block
			if ((error = this.parseG(nodes[index])) != null) return error;
		}
	...
	parseG(gNode) {
		...
		this.defaultCameraId = this.reader.getString(gNode, "default");
		...
		for (let i = 0; i < children.length; i++) {
			...
			// path view
			if (view.nodeName == "path") {
				// near
				var near = this.reader.getFloat(view, "style");
				if (!(near != null && !isNaN(near)))
					return "unable to parse style of the view for ID = " + viewId;

				// far
				var far = this.reader.getFloat(view, "id");
				if (!(far != null && !isNaN(far)))
					return "unable to parse id of the view for ID = " + viewId;

				// angle
				var angle = this.reader.getFloat(view, "d");
				if (!(angle != null && !isNaN(angle)))
					return "unable to parse d of the view for ID = " + viewId;
